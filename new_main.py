...
# Apply the custom style for radio buttons
    ...
    if tabs == "**User**":
        ...
        with st.sidebar:
            ...
            uploaded_files = st.file_uploader("**Choose your files**", type=["pdf", "csv", "xlsx", "docx", "pptx"], accept_multiple_files=True,disabled=bool(customer_self_demo_flg))
...
def query_page(collection_name, db_path, admin):
    ...
    if customer_self_demo_flg:
        ...
        pre_defined_questions=st.selectbox("Some frequently asked questions", questions)   
        FAQ=None
        if st.button("Choose"):
        ...
        if single_question:
            ...
            with st.spinner("Generating response..."):
                    ...
                    if 'documents' in collection.get() and len(collection.get()['documents']) > 0:
                        ...
                        context_str = "\n\n".join([r.get_content().replace('{', '').replace('}','') for r in retrieved_nodes])
